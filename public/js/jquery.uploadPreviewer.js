...
(function($) {
  defaults = {
    ...
    rowTemplate: function(options) {
      return "<tr class='" + config.tableRowClass + "'>" +
	       // "<td width='20%'>" + "<img src='" + options.src + "' class='" + options.placeholderCssClass + "' />" + "</td>" +
	       "<td width='20%'>" + "<img src='" + basePath + "files/document-file/baseline_picture_as_pdf_black_48dp.png' class='" + options.placeholderCssClass + "' />" + "</td>" +
	       "<td class='filename'>" + options.name + "</td>" +
	       "<td class='filesize' width='10%'>" + options.size + "</td>" +
	       "<td class='remove-file' width='5%'></td>" +
	       // "<td class='remove-file' width='5%'><button class='btn btn-danger'><i class='glyphicon glyphicon-remove'></i> </button></td>" +
	     "</tr>";
    },
  ...
  if(typeof Humanize == 'undefined' || typeof Humanize.filesize != 'function'){
	...
	$.getScript("/js/customize/ajax/humanize.min.js")
  ...
  $.fn.uploadPreviewer = function(options, callback) {
	...
	var filesCount = e.currentTarget.files.length;
  	...
  	previewTableBody.empty();//empty table body when browse file gain
